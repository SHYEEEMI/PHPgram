const feedObj = {
    ...
    makeCmtItem: function(item){
        ...
        const src = '/static/img/profile/' + (item.writerimg ? `${item.iuser}/${item.writerimg}`:'bear.png');
        divCmtItemContainer.innerHTML =`
            <div class="circleimg h24 w24 me-1">
                <img src="${src}" class="profile w24 pointer profileimg">
            </div>
            <div class="d-flex flex-row align-items-center">
            <div class="pointer me-2">${item.writer} <span>${item.cmt}</span></div>
                <div class="rem0_7">${getDateTimeInfo(item.regdt)}</div>
            </div>
        `;
    ...
    makeFeedItem: function(item) {
        ...
        const writerImg = `<img class="profileimg" src='/static/img/profile/${item.iuser}/${item.mainimg}' 
            onerror='this.error=null;this.src="/static/img/profile/bear.png"'>`;
